...
return [latitude , longitude]
        ...
        async  function  fetchData(){
            var arr =[]
            await axios.get("http://192.168.22.163:5000/admin/getAllMechanic").then(res=> {
...
console.log("Test")
            for (var  i = 0 ;i< res.data.length;i ++){  
             ...
             markers[i].place= res.data[i].namePlace ; 
                 markers[i].address=res.data[i].adress 
                  ...
                  var rate = (((res.data[i].stars/res.data[i].pepleCount ).toFixed(2)))
                  markers[i].rating = rate 
                  markers[i].count = res.data[i].pepleCount 
